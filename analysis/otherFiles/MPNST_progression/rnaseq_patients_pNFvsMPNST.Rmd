...
knitr::opts_chunk$set(echo = TRUE, tidy.opts=list(width.cutoff=50), tidy=TRUE, fig.width=12, fig.height=8,fig.path = "/Users/xdoan/Shell/R/MPNSTAnalysis/analysis/MPNST_progression/figs/", dev=c('png', 'pdf'))
...
write.csv(res_df, file = "MPNST_progression/output/pNF_v_MPNST_geneList.csv", row.names = TRUE)
```
...
```{r}
write.csv(ego,file="MPNST_progression/output/pNFvMPNST_GO_overRep.csv", row.names = FALSE)
...
```{r}
write.csv(kk,file="MPNST_progression/output/pNFvMPNST_KEGG.csv", row.names = FALSE)
```
